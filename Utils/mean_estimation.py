...
def filter_MMW(X, alpha, T1, T2, epsilon, delta, C, B):
    ...
    for _ in range(T1):
        ...
        if lamb_s < C * alpha * np.log(1 / alpha):
            output = np.mean(X[S], axis=0) + np.random.normal(0, 2 * B * np.sqrt(2 * d * np.log(1.25 / delta_1)) / (
                        n * epsilon_1), d)
            print('succeded, ' + str(S.max()) + str("  ") + str(LA.norm(output)) + '  ' + str(
                np.sum(S >= n - int(alpha * n))))
            return output
        ...
        for _ in range(T2):
            ...
            if lamb_t < lamb_s * 0.5:
                ...
                Sigma = M(X[S], n) + output_perturbation(
                    2 * B * B * d * np.sqrt(2 * np.log(1 / delta_2)) / (n * epsilon_2), d)
                Sigma_list.append(Sigma)
                sum_sigma = alpha_s * (np.array(Sigma_list) - np.identity(d)).sum(0)
                U = np.exp(sum_sigma) / np.trace(np.exp(sum_sigma))
                
                psi = np.trace((M(X[S], n) - np.identity(d)) @ (U.T)) + np.random.laplace(0, 2 * B ** 2 * d / (
                            n * epsilon_2))
                if psi <= lamb_t / 5.5:
                    continue
                else:
                    mu_t = np.mean(X[S], axis=0) + np.random.normal(0, 2 * B * np.sqrt(2 * d * np.log(1 / delta_2)) / (
                                n * epsilon_2), d)
...
if __name__ == "__main__":
    ...
    # plt.errorbar([1, 10, 20, 30, 40, 50, 75, 100], errors_more.mean(axis=1),
    #              np.std(errors_more, axis=1) / np.sqrt(trials), marker='o', label='DP mean')
    # plt.errorbar([1, 10, 20, 30, 40, 50, 75, 100], errors_prime_more.mean(axis=1),
    #              np.std(errors_prime_more, axis=1) / np.sqrt(trials), marker='D', label='Prime')
    ...
    # X = generate_dataset(d=10, n=1000000, alpha=0.05, shift=1.5)
    ...
    real_mean = np.mean(X_good, axis=0)
    bad_mean = np.mean(X_bad, axis=0)
    origin_mean = np.mean(X, axis=0)
    diff = np.sum(np.abs(real_mean - origin_mean))
    print(f"{diff = }")
